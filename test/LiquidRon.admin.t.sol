...
contract LiquidRonTest is Test {
    ...
    function test_admin_fetch_operator_fee(uint88 _amount) public {
        ...
        uint256 delta = newTotal - total;
        uint256 tolerance = (expectedYield / 1e9) + 1;
        uint256 minExpected = expectedYield > (expectedFee + tolerance) ? expectedYield - expectedFee - tolerance : 0;
        uint256 maxExpected = expectedYield + tolerance;
        assertTrue(delta >= minExpected);
        assertTrue(delta <= maxExpected);
    ...
    function test_revert_fetch_fee_non_receiver(uint88 _amount) public {
        ...
        if (liquidRon.operatorFeeAmount() > 0) {
